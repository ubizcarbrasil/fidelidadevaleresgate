...
export default function DriverDetailSheet({ driver, brandId, onClose }: Props) {
  ...
  // Renderização condicional: o Sheet só existe quando há motorista.
  // Isso evita que o Radix mantenha um portal vazio que conflita com a animação
  // mobile (causa raiz do "removeChild is not a child of this node").
        ...
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto p-4 sm:p-6 pwa-safe-bottom">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2 text-base sm:text-lg pr-6">
              ...
              <span className="truncate">{cleanName(driver.name)}</span>
            </SheetTitle>
          </SheetHeader>
