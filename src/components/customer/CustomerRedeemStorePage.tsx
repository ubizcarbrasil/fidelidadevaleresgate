...
import { LucideIcon, formatPrice } from "@/components/driver/DriverMarketplace";
import CustomerRedeemCheckout from "./CustomerRedeemCheckout";
...
export default function CustomerRedeemStorePage({ onBack }: Props) {
  const { brand, selectedBranch, theme } = useBrand();
  const { customer } = useCustomer();
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedCatId, setSelectedCatId] = useState<string | null>(null);
  const [redeemDeal, setRedeemDeal] = useState<RedeemDeal | null>(null);
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // If not mirroring, only show customer/both products
      if (!mirrorDriver) {
        q = q.in("redeemable_by", ["customer", "both"]);
      }
      ...
      {redeemDeal && (
        <CustomerRedeemCheckout
          deal={{
            ...
            redeem_points_cost: redeemDeal.redeem_points_cost || Math.ceil(redeemDeal.price || 0),
          }}
