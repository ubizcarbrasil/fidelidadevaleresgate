...
export default function NotificationDrawer({ open, onClose }: NotificationDrawerProps) {
  ...
  const handleNotificationClick = async (notif: CustomerNotification) => {
    ...
    if (notif.reference_type === "offer" && notif.reference_id) {
      // Open offer detail - we pass a minimal object, the detail page will fetch full data
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-8">
              {notifications.length === 0 ? (
                <div className="text-center py-12 opacity-40">
                  <Bell className="h-10 w-10 mx-auto mb-3" style={{ color: `${fg}30` }} />
                ...
                <div className="space-y-1">
                  {notifications.map((notif) => (
                    <motion.button
                      ...
                      style={{
                        backgroundColor: notif.is_read ? "transparent" : `${primary}06`,
                      }}
                    >
                      <div
                        className="h-9 w-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5"
                        style={{
                          backgroundColor: notif.is_read ? `${fg}08` : `${primary}12`,
                          color: notif.is_read ? `${fg}40` : primary,
                        }}
                      >
                        {getIcon(notif.type)}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] mt-1" style={{ color: `${fg}35` }}>
