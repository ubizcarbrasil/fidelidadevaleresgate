...
function CRMPhoneMockup() {
  return (
    ...
    >
      <div className="absolute -inset-8 rounded-[4rem] bg-emerald-500/10 blur-3xl -z-10 animate-pulse" />

      <div className="rounded-[2.5rem] border-[5px] border-white/15 bg-[hsl(160,30%,10%)] shadow-[0_25px_60px_-15px_rgba(16,185,129,0.2)] overflow-hidden">
        <div className="flex justify-center pt-2 pb-1 bg-white/5">
          <div className="w-20 h-5 rounded-full bg-white/10" />
        </div>

        <div className="h-[370px] md:h-[400px] bg-gradient-to-b from-emerald-500/8 via-[hsl(160,30%,10%)] to-[hsl(160,30%,10%)] p-3.5 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <motion.div
              className="h-7 w-7 rounded-full bg-emerald-500/15 flex items-center justify-center"
          ...
          <motion.div
            className="rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-600 p-3.5 text-white shadow-lg"
            initial={{ scale: 0.95 }}
            whileInView={{ scale: 1 }}
            viewport={{ once: true }}
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: "Ativos", value: "342", icon: Users, color: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
              ...
              { label: "Potenciais", value: "67", icon: Target, color: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
            ].map((s, idx) => (
              <motion.div
                key={s.label}
...
export default function LandingCRM() {
  return (
    <section id="crm" className="py-24 md:py-32 bg-[hsl(160,30%,6%)] relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-5 relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} className="max-w-6xl mx-auto">
          <motion.div variants={fadeUp} custom={0} className="text-center mb-8">
            <span className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-emerald-500/10 text-emerald-400 text-sm font-bold border border-emerald-500/20">
          ...
          <motion.h2 variants={fadeUp} custom={1} className="text-4xl md:text-6xl font-black text-center text-white mb-5 leading-tight">
            CRM Inteligente que{" "}
            <span className="text-emerald-400">recupera clientes</span>
          </motion.h2>
            ...
            <div className="flex-1 space-y-5">
              {crmFeatures.map((feature, i) => (
                <motion.div
                  ...
                  variants={fadeUp}
                  custom={3 + i}
                  className="flex gap-4 items-start rounded-2xl bg-white/[0.03] border border-white/[0.06] p-5 hover:border-emerald-500/20 transition-colors"
                >
              ...
              <motion.div variants={fadeUp} custom={6} className="pt-6 flex flex-col sm:flex-row gap-3">
                <Button
                  size="lg"
                  className="rounded-full px-10 py-7 text-base font-bold bg-emerald-500 hover:bg-emerald-400 text-white shadow-[0_8px_30px_-5px_rgba(16,185,129,0.4)] border-0"
