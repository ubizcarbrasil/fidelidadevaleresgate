...
-- Fix RLS: drop overly permissive policy and create proper ones
DROP POLICY "Authenticated users can manage categories" ON public.affiliate_deal_categories;

CREATE POLICY "Authenticated can insert categories"
  ON public.affiliate_deal_categories FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Authenticated can update categories"
  ON public.affiliate_deal_categories FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Authenticated can delete categories"
  ON public.affiliate_deal_categories FOR DELETE
  TO authenticated
  USING (true);

-- Create a function to seed default categories for a brand
...
AS $$
BEGIN
  -- Only seed if brand has no categories yet
  ...
  INSERT INTO affiliate_deal_categories (brand_id, name, icon_name, color, order_index, keywords) VALUES
    ...
    (p_brand_id, 'Mercado', 'ShoppingBasket', '#10b981', 8, ARRAY['mercado','alimento','bebida','supermercado','comida','snack','chocolate','cafe']),
