-- Ampliar event_types aceitos em campeonato_notifications
ALTER TABLE public.campeonato_notifications
  DROP CONSTRAINT IF EXISTS duelo_notifications_event_type_check;

ALTER TABLE public.campeonato_notifications
  ADD CONSTRAINT duelo_notifications_event_type_check
  CHECK (event_type = ANY (ARRAY[
    'season_created'::text,
    'knockout_started'::text,
    'match_result'::text,
    'prize_received'::text,
    'duelo_win'::text,
    'duelo_loss'::text,
...
-- Reescrever RPC: apenas o hook de notificação de match_result muda.
-- Toda a lógica de avanço, brackets, prêmios e standings preservada.
