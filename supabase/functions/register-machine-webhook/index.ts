...
Deno.serve(async (req) => {
  ...
  const { data: { user: callerUser }, error: userErr } = await sb.auth.getUser(token);
  ...
  try {
    ...
    const { brand_id, branch_id, api_key, basic_auth_user, basic_auth_password, action, matrix_api_key, matrix_basic_auth_user, matrix_basic_auth_password, telegram_chat_id } = body;
